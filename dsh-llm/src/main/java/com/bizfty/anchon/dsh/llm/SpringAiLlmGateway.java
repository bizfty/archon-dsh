...
public class SpringAiLlmGateway implements LlmGateway {
    ...
    private static final String[] AUTH_ERROR_MARKERS = {
            "401", "403", "unauthorized", "authentication fails", "invalid api key", "invalid api_key",
            "api key", "apikey", "auth", "forbidden", "invalid_api_key", "insufficient"
    };

    /** 检测异常链中是否包含认证/授权错误，是则抛 {@link LlmAuthException}。 */
    static void throwIfAuthError(Throwable e) {
        Throwable cur = e;
        while (cur != null) {
            String msg = cur.getMessage();
            if (msg != null) {
                String lower = msg.toLowerCase();
                for (String marker : AUTH_ERROR_MARKERS) {
                    if (lower.contains(marker)) {
                        String safeMessage = sanitizeMessage(msg);
                        throw new LlmAuthException(safeMessage, e);
                    }
                }
            }
            cur = cur.getCause();
    ...
    private static String sanitizeMessage(String raw) {
        String safe = raw.replaceAll("(?i)(api[_-]?key\\s*[:=]\\s*)[\\w\\-.]+", "$1****");
        if (safe.length() > 500) {
